...
앱 실행 후에는 **설정 > 서버** 화면에서 모드 전환, 원격 주소 입력, 연결 테스트, `이 기기`
모드 복귀를 할 수 있다. 저장은 `server.json`을 갱신하고, 적용은 앱을 다시 시작한 뒤부터다
(같은 화면의 "지금 다시 시작" 버튼). `server.json`을 직접 만드는 방식은 첫 실행 전 준비용으로
남는다.
...
앱 내부 서버 연결 설정·연결 테스트·`이 기기` 복귀 화면은 **설정 > 서버**로 구현되어 있다
(위 §7 참고). 남은 후속 작업은 원격 모드 자격증명/토큰 인증(현재는 네트워크 레벨만)과
오프라인 동기화다.
